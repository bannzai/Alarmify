...
- 超過後はどの通知にも同じ閾値が載るため、集計期間 (`costIntervalStart`。月次予算では月初) ごとに投稿済みの最大閾値を `budgetNotifications/{budgetId}` に覚え、閾値が上がった時 (50% → 90% → 100%) だけ投稿する。月が変わると記録を捨てて 50% から投稿し直す。記録済みより古い集計期間の通知 (月替わり後に遅れて届いた前月分) は投稿せず、記録も巻き戻さない (Pub/Sub の配送順序に依存しない)
