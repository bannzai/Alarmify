...
| ユーザー数 | Pro 比率 | 月間アラーム | GCP 変動費 + 定期実行 | 固定費の月割り (Apple + ドメイン) | 月額合計 |
| --- | --- | --- | --- | --- | --- |
| 0〜100 | 0〜5% | 〜3,400 | ¥30〜70 | ¥1,410 | 約 ¥1,450〜1,480 |
| 1,000 | 5% | 34,000 | ¥345 | ¥1,410 | 約 ¥1,750 |
| 10,000 | 5% | 340,000 | ¥3,100 | ¥1,410 | 約 ¥4,500 |
| 10,000 | 100% (最悪) | 3,000,000 | ¥27,000 | ¥1,410 | 約 ¥28,400 |

効いてくる変数:

| 変数 | 試算の仮定 | 動くと | 影響の大きさ |
| --- | --- | --- | --- |
| Cloud Run の課金時間 / リクエスト (t) | 1.0 秒 (本番の実測。コールドスタート込み) | 暖機で 0.2 秒 | GCP 分が約 1/3〜1/5。最大の変数 |
| 1 人あたりの月間アラーム数 | 無料 20 件、Pro 300 件 | Pro が 300 件より多い / 少ない | 変動費に比例 |
| Pro 比率 | 5% | 100% | 変動費が約 9 倍 (Pro は無料の 15 倍登録する仮定) |
...
| --- | --- | --- | --- |
| 15% | ¥1,745 / 年 | Pro 10.0 人 → 10 人 | Pro 10.2 人 → 11 人 (全体 約 220 人) |
| 30% | ¥1,432 / 年 | Pro 12.2 人 → 13 人 | Pro 12.5 人 → 13 人 (全体 約 260 人) |
...
| 期限切れの削除 (Firestore の TTL ポリシー `alarms` / `expiresAt`。[ADR 0007](adr/0007-delete-expired-alarms-with-firestore-ttl.md)) | `firebase/firestore.indexes.json` の `ttl: true`。件数の上限なし、削除は期限から通常 24 時間以内 | 期限切れの滞留。月 300 万件 (Pro 300 件/月 × 10,000 人。新プランの Pro 1000 件/月なら 3,000 人相当) でも 1 日 10 万件を Firestore 側が消し、費用は $0.013 / 100,000 件 × 30 = 約 $0.39 / 月 (TTL の削除は無料枠の対象外。単価は東京の Document Deletes と同額の仮定。根拠は ADR 0007)。旧方式 (Scheduled Function が 6 時間毎に最大 10,000 件 = 月 120 万件) の同規模の費用 (read 約 $1.14 + Cloud Run 約 $0.2 + 削除 約 $0.31) より安い | 削除までの最長 24 時間は履歴に期限切れが残る。TTL が消し残した分 (期限から 48 時間超) は `cleanupExpiredAlarms` (1 日 1 回、最大 10,000 件) が削除し、1 件でもあれば error ログ → アラートポリシー `expired-alarms-outlived-ttl` (`firebase/monitoring/`) が Slack + メールへ通知する |
...
| Pro トークンで別 id のアラームを登録し続ける (登録 + push: 6 read、4 write、1 リクエスト、保存。端末の報告と定期削除を除く) | 864,000 回 | 約 $11〜28 (¥1,600〜4,200。t = 0.2〜1.0 秒)。端末 1 台の報告と定期削除まで含めると 1 アラームあたりの費用 × 864,000 = 約 $18〜52 (¥2,700〜7,800) |
| 形式は正しい未知トークンを送り続ける (1 read) | 8,640,000 回 (6,000 req/分) | 約 $28 (¥4,200)。内訳: 読み取り $3.28 + リクエスト $3.46 + CPU・メモリ $21.28 (1 件 0.2 秒 × 864 万件 = 172.8 万秒だが、10 インスタンス × 86,400 秒 = 864,000 秒で頭打ち) |
| 上記に加えて、レート制限で 429 になる呼び出し (Firestore なし。1 件あたり約 $0.000002 = リクエスト $0.0000004 + 処理 50 ms) | 制限なし (受信した分だけ) | 100 万件あたり約 $2 (¥300)。CPU + メモリは最大 10 インスタンスが 1 日中処理中でも約 $21 (¥3,200) / 日が上限で、リクエスト課金だけが件数に比例して増える |

いずれも 1 日で予算 ¥10,000 の 50% (¥5,000) に届くか、数日で届く。予算の通知は早くて数時間〜1 日遅れるため、通知を受けてからトークンを失効 (`DELETE /v1/api-tokens/{id}`) するまでの間の費用は数千円のオーダーになる。トークン漏洩の実害は費用より「本物のアラームが鳴り続ける」ことである。トークンの失効は `revokedAt` を更新して新規の登録・取り消しを止めるだけで、漏洩中に登録された未来のアラームは残り、cancel の push も送られない。登録済みのアラームは別途、有効な (新しく発行した) トークンで `DELETE /v1/alarms/{id}` を呼ぶか、端末のアプリ側で取り消す。
...
- 定期削除の処理能力 (月 120 万件) が Pro 4,000 人相当で頭打ちになる。別 issue ( https://github.com/bannzai/Alarmify/issues/74 ) で Firestore の TTL ポリシーへ切り替え、上限を無くした ([ADR 0007](adr/0007-delete-expired-alarms-with-firestore-ttl.md)。上表の `cleanupExpiredAlarms` の行は切り替え前の確認結果)
