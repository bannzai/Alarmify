...
リポジトリ・GitHub・GCP の点検結果を以下に記録した。**公開前点検全体は未完了**。指定された Claude Code `/security-review` は初回が無出力のため停止 (exit 143)、再実行が自動承認レビューにより拒否された。コードの外部サービスへの送信承認が理由であり、指摘ゼロのレビュー結果ではない。以下のコード所見はファイルを直接確認した静的点検である。

秘密鍵・実運用の API トークンの混入は今回の走査で確認しなかった。ただしスキャナーの生の結果は NG で、テスト用文字列・公開済み Firebase キー・環境設定の検出を含む。iOS キーの bundle ID 制限は実設定で確認済み。権限の広さ、依存の警告、Privacy Manifest、CI の参照固定には残課題がある。

本番の配置は現在のソースと異なる。点検を完了することと、本番で防御が有効になることを分けて扱う。負荷試験・侵入試験・実機操作・本番への書き込み・デプロイ・権限変更は実施していない。
...
| App Check | 現在のソースは App Check → IDトークンの順で検証。`lib/appCheck.ts` は monitor / enforce を実装し、不明なモード値は monitor。追跡済み `.env.alarmify-prod` は enforce | 本番 Cloud Run の環境変数に同キーは無く、5サービスは9月3日の配置。現時点を「強制適用済み」としない。#58 が配置とモードの整合を扱う |
| 削除中の保護 | `api/appApi.ts` は端末登録・トークン発行・端末反映結果の書き込みトランザクションで `deletedAccounts/{uid}` を読み、410で拒否。削除目印は最低2時間保持 | 本番への最新反映と、#58 に記録された競合テスト失敗の解消が必要 |
| Callable deleteAccount | `account/deleteAccount.ts` は request.auth.uid だけを削除対象にする。認証なしは拒否し、削除の失敗を sweep で回復する。`index.ts` の enforceAppCheck は配置時のモードに連動 | 実際のアカウント削除は点検で実行しない。App Check の本番配置は #58 |
| RevenueCat webhook | Authorization が空・不一致なら拒否。等長バッファは `timingSafeEqual`、異長は即拒否なので「長さを含む完全な定数時間」ではない。古いイベント・削除中/削除済みユーザーへの再作成を防ぐ処理あり | `REVENUECAT_WEBHOOK_AUTHORIZATION` の Secret 名は存在するが、本番に関数が無い。Secret の値や Dashboard との一致は未確認。#58 / #25 |
...
- Cloud Run の全5サービスは ingress=ALLOW_ALL。`alarmsapi` / `appapi` / `deleteaccount` は allUsers の run.invoker があり、アプリケーション内の認証を入口にする構成。定期処理2サービスは実行用 SA の run.invoker のみで、allUsers は無い。Scheduler は両方 ENABLED で OIDC の SA 指定あり。
- 実行 SA は5関数とも既定 Compute Engine SA。そのプロジェクト権限に Editor、Cloud Build、Artifact Registry の権限が混在。デプロイ SA も cloudfunctions.admin / firebase.admin / cloudscheduler.admin / secretmanager.admin をプロジェクト単位で保持。最小権限とは判定しない (#66)。親階層の継承 IAM と全 Secret 個別 IAM までは棚卸ししていない。
...
| 対応 | Issue |
| --- | --- |
| 指定の Claude セキュリティレビューを承認後に実行し、本書を更新 | #63 (未完了のまま維持)、操作の依頼は #25 |
| 最新 Functions の配置、App Check のモードと監視方針の整合、競合テストの解消、webhook の配置 | https://github.com/bannzai/Alarmify/issues/58 |
| webhook Dashboard の設定 | https://github.com/bannzai/Alarmify/issues/25 |
| 費用・流量の許容範囲、インスタンス内レート制限を超える防御の必要性 | https://github.com/bannzai/Alarmify/issues/62 |
| Manifest の不足・App Group の理由・SDK 集約・ASC 回答の一致 | https://github.com/bannzai/Alarmify/issues/64 |
| 実行・ビルド・デプロイ IAM の縮小 | https://github.com/bannzai/Alarmify/issues/66 |
| 開発依存の脆弱性 | https://github.com/bannzai/Alarmify/issues/67 |
| CI action の SHA 固定 | https://github.com/bannzai/Alarmify/issues/68 |
