...
- **滞留の検知**: 予備の経路が 1 件でも削除したら error ログ `expired alarms outlived the TTL policy` (識別子 `jsonPayload.event="expired-alarms-outlived-ttl"`) を出し、その識別子に一致するログ条件のアラートポリシー (`firebase/monitoring/expired-alarms-outlived-ttl.policy.json`。通知先は既存の Slack `#alarmify-notification` + メール) で通知する。既存の `error-log-spike` は「5 分間に 10 件超」が条件で 1 行の error では発火しないため、別に持つ。ポリシーは `firebase/monitoring/apply-policy.sh` で冪等に適用する
...
**悪い点 / 引き受けるリスク:**
- 削除は best effort で、期限からの遅れに保証された上限が無い。Firestore の TTL は「通常は 24 時間以内」で、予備の経路も 1 日 1 回・最大 10,000 件のため、TTL の遅延や消し残しがそれを超えると期限切れがさらに残る。履歴 `GET /v1/alarms` は `expiresAt` を条件にしていないため、削除されるまで期限切れが履歴に残る。公開文書の「30 日で削除」は実態として「30 日を過ぎた後に順次削除 (通常は 1 日以内)」になる。期限を保証したくなったら、応答側で `expiresAt` 経過分を除く (履歴と取り消しからは即座に消える) か、予備の経路の上限・頻度を上げる
