...
1. **セットアップ**: アプリでアラームの権限を許可し、API トークンを発行する (アカウントは Firebase Auth の匿名認証で自動作成し、Firebase Auth の keychain 永続化で同じ端末の再インストールを跨ぐ。匿名のままでも使い続けられ、複数の iPhone を 1 つのアカウントにまとめたい時や機種変更で引き継ぎたい時に任意で Sign in with Apple する。2 台目の追加には Pro が要るため、Pro の購入時は Sign in with Apple を必須にする (ペイウォールの購入ボタンで未連携なら購入の前にサインインを求め、統合先が既に Pro なら購入しない。メールアドレスと氏名は要求しない。求めるのは購入時だけで、購入の復元と、匿名のまま Pro を購入済みのユーザーには求めない)。Sign in with Apple をしていない端末の買い替えは、新端末で新しい無料ユーザーになりトークンを発行し直す)
...
| 層 | 内容 | 狙い |
| --- | --- | --- |
| 無料 | API トークン 1 つ・月 50 回までのアラーム登録・受信端末 1 台 (最初に登録した端末)・アプリ内からのテストアラーム | 「Webhook で本物のアラームが鳴る」体験を完全に試せる線 |
| Pro (年 $14.99 目安。月額は $1.99〜2.99 目安) | 複数トークン (サービス別)・月 1000 回までの登録・アラーム履歴・登録済みの全端末 (上限 20 台) への配送 | サーバー側で価値を出す機能を Pro に寄せ、サブスクに値する理由を明確にする |

価格の確定と RevenueCat の商品設定は公開前チェックリストで扱う。プラン別の端末数・端末の紐付け方 (Sign in with Apple)・配送先と失効時の挙動・採算の試算は 2026-09-13 に決めた。
