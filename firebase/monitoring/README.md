...
| ファイル | 発火条件 | 見るもの |
| --- | --- | --- |
| `expired-alarms-outlived-ttl.policy.json` | `cleanupExpiredAlarms` が「TTL の猶予 48 時間を過ぎた期限切れ」を 1 件でも削除した (error ログ `expired alarms outlived the TTL policy`) | TTL ポリシーの状態と消し残しの件数 ([ADR 0007](../../documents/adr/0007-delete-expired-alarms-with-firestore-ttl.md)) |
