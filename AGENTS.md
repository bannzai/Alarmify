...
- 課金商品の登録: App Store Connect のサブスクリプションは `fastlane/in_app_purchases/appstore.config.json`、RevenueCat の products / entitlement / offering は `fastlane/in_app_purchases/revenuecat.config.json` が正で、それぞれ `/appstore-in-app-purchase` (`iap_apply_subscriptions.sh --config <ファイル>`) と `/revenuecat-product-setup` (`rc_apply_config.sh <ファイル>`) で冪等に適用する (適用コマンドは各ファイルの `comment`)。商品 ID・価格は `Alarmify.storekit` と一致させる
...
- 本番画面の翻訳品質チェックは `.claude/skills/detect-mistranslation/SKILL.md`、撮影対象の追加は `scripts/snapshot_ui_tests/README.md` に従う
...
- `docs/` は Cloudflare Pages (プロジェクト `signalarm`、https://signalarm.app/ ) で配信する LP・法務ドキュメント・API リファレンス・連携レシピ。`scripts/site/build.mjs` が Markdown を HTML 化して `tmp/site` を作り、main へのマージで `.github/workflows/pages-deploy.yml` が配信する。外部サービス向け API の公開ホスト `api.signalarm.app` は `scripts/api-proxy/` の Cloudflare Worker が Functions の `alarmsApi` へ中継する (方式: [ADR 0006](documents/adr/0006-api-domain-via-cloudflare-worker.md)。配信は `.github/workflows/api-proxy-deploy.yml`)。LP の検証は `bash ~/.agents/skills/landing-page-builder/scripts/verify-lp.sh --app-store-support --has-account docs/index.html`
- Functions の構成: アプリ向け API `appApi` (App Check トークンと Firebase Auth の ID トークンで認証。端末登録・API トークンの発行 / 失効・アラーム履歴)、外部サービス向け API `alarmsApi` (Bearer = API トークン。`POST /v1/alarms` / `DELETE /v1/alarms/{id}`)、保持期間 (30 日) を過ぎたアラーム要求のうち Firestore の TTL ポリシーが消し残したものを消してアラートを出す `cleanupExpiredAlarms` (削除の主経路は TTL。[ADR 0008](documents/adr/0008-delete-expired-alarms-with-firestore-ttl.md))、アカウント削除の Callable `deleteAccount` とその掃除を完了させる `sweepDeletedAccountsHourly` (削除中の目印 `deletedAccounts/{uid}` がある間は `appApi` の書き込みを 410 で拒否し、目印は 2 時間後の sweep が最後の掃除と一緒に消す)、RevenueCat の webhook を受けて `users/{uid}.plan` を更新する `revenueCatWebhook` (Authorization ヘッダーの値は Secret `REVENUECAT_WEBHOOK_AUTHORIZATION`。設定手順と方式の選定は `documents/revenuecat-webhook.md` と [ADR 0005](documents/adr/0005-sync-plan-with-revenuecat-webhook.md)。無料枠の判定は `plan` と `proExpiresAt` から `effectivePlan` で決める)、Cloud Billing の予算通知 (Pub/Sub トピック `budget-alarmify-prod`) を Slack `#alarmify-notification` へ転送する `budgetAlertToSlack` (bot token は Secret `SLACK_BOT_TOKEN`。設定手順と方式の選定は `documents/budget-alert-slack.md` と [ADR 0007](documents/adr/0007-budget-alert-to-slack-via-pubsub-function.md))。push payload は `Alarmify/Shared/AlarmRequest.swift` の形式に揃え、`mutable-content` と `content-available` の切り替えは環境変数 `ALARMIFY_PUSH_DELIVERY` (`notification-service` / `background`) で行う
